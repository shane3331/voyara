...
module.exports = async (req, res) => {
  if (guard(req, res, { limit: { name: 'cancel', max: 10, windowMs: 60000 } })) return;
  ...
  // Never the email in the body. A cancellation is destructive and the caller
  // must prove who they are, or a booking id is enough to cancel a stranger's
  // holiday.
  const caller = dbConfigured() ? await verifyCaller(req) : null;
  if (dbConfigured() && !caller) {
    return res.status(401).end(JSON.stringify({
      error: 'unauthorized', detail: 'Sign in to cancel a booking.'
    }));
  }
  const email = caller ? caller.email : String(b.email || '').trim().toLowerCase();
